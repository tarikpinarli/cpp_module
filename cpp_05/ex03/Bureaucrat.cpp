...
Bureaucrat::~Bureaucrat() {
    std::cout << RED << "[Bureaucrat] \"" << _name << "\" destroyed." << std::endl;
}
...
void Bureaucrat::signForm(AForm& form) const {
    ...
    try {
        form.beSigned(*this);
        std::cout << this->getName() << " signed \"" << form.getName() << "\" form."<< "\n";
    } catch (const std::exception& e) {
        std::cout << RED << "Signiture authority exception caught: " << WHITE << this->getName() << " couldn't sign \"" << form.getName()
                  << "\" form because " << e.what() << "\n";
    }
}
...
void Bureaucrat::executeForm(const AForm& form) const {
    try {
        form.execute(*this);
        std::cout << GREEN << this->getName() << " executed \"" << form.getName() << "\" form succesfuly!" << std::endl;
    } catch (const std::exception& e) {
        std::cout << RED << "Execution exception caught: " << WHITE << this->getName() << " couldn't execute \"" << form.getName()
                  << "\" form because " << e.what() << "\n";
    }
}
